...
The accumulated cost $D(N, M)$ grows mechanically with the duration of the
sequences being compared, which can bias a nearest-neighbour classifier when
gestures differ in length. We therefore normalise $D(N, M)$ by the length $L$
of the optimal warping path, recovered via backtracking from $(N, M)$ to $(1, 1)$:
\begin{equation}
\label{eq:dtw-norm}
    D_{\text{norm}}(N, M) = \frac{D(N, M)}{L} .
\end{equation}

The final similarity score $D_{\text{norm}}(N, M)$ is then plugged into a
$k$-nearest-neighbors classifier: the test gesture is assigned the majority
label among its $k$ closest templates.
...
final value $D(|S_1|, |S_2|)$ gives the (unnormalised) edit distance between
the two complete sequences.

\paragraph{Length normalisation.} As with DTW, the raw edit distance
grows mechanically with the length of the symbolic sequences being compared,
which can bias the nearest-neighbour classifier when sequences differ in
length --- in particular when the optional consecutive-duplicate compression
step is applied, since it shortens sequences by a variable amount. We
therefore normalise the edit distance by the length of the longer sequence:
\begin{equation}
\label{eq:ed-norm}
    d_{\text{norm}}(S_1, S_2) =
    \frac{D(|S_1|, |S_2|)}{\max(|S_1|, |S_2|)} \in [0, 1] .
\end{equation}
The normalised distance lies in $[0, 1]$, where $0$ indicates identical
sequences and $1$ indicates that every symbol must be edited.

Classification is then performed with a $k$-nearest-neighbours rule, assigning
each test gesture the majority label among its $k$ nearest templates in the
$d_{\text{norm}}$ metric.
