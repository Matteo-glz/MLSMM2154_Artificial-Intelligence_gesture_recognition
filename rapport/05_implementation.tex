...
\begin{itemize}\itemsep 0em
    ...
    $T \in \{16, 32, 64\}$ and the number of hidden units per
    direction $h \in \{16, 32, 64\}$ were tuned by grid search.
    The following architectural parameters were fixed throughout:
    dropout rate $p = 0.30$, batch size $= 16$, maximum epochs
    $= 50$ with early stopping (patience $= 5$, monitoring
    validation loss). No PCA was applied.
\end{itemize}
